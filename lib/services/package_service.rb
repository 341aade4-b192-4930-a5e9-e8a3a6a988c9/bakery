class PackageService
  ...
  def try_get_first_pack(list_of_packs, number_of_items)
    solution =
        find_solution(list_of_packs, number_of_items - list_of_packs.first.count)

    solution[list_of_packs.first.count] += 1 if solution
  end
  ...
  def try_get_next_pack(list_of_packs, number_of_items)
    ...
    solution =
        find_solution(list_without_first_option, number_of_items - list_without_first_pack.first.count) if list_without_first_pack.any?

    solution[list_without_first_pack.first.count] = 1 if solution
  end
